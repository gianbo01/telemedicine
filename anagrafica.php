	...
	<body>
		...
		<?php
			...
			if (isset($_POST['submitPaziente'])){
				...
				$sql = "SELECT MAX(num) AS n FROM `anagrafica`;";
				...
				$sql = "INSERT INTO `anagrafica`(`ID`, `nome`, `cognome`, `nome_madre`, `sesso`, `dataNascita`, `provenienza`, `num_tenda`, `sezione`)
					VALUES ('$id','$nome','$cognome','$nome_madre','$sesso','$dataN','$prov','$num_tenda','$sezione')";
			...
			elseif (isset($_POST['addInfoCliniche'])){
				var_dump($_POST);
				...
				// crea l'identificativo del paziente con [NNN][CCC][#indice]
				$id = substr($nome,0,3).substr($cognome,0,3).$n;
				$id = strtoupper($id);
				echo 'ID: '.$id;
				...
				header('Location: addInfoCliniche.php?ID='.$id);
			}
